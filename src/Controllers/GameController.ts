...
export const saveStats = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const errorsArray = await Error_Keys.bulkCreate(errors);

    const registersArray = await Registros.bulkCreate(registers);
...
export const getAllStats = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const UserStats = await User_Statistics.findAll({
